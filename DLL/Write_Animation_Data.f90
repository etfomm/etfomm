...
#else
            CALL WRITE_VEHICLE_TEXT(THELINKDATA%USN, THELINKDATA%DSN, THEVEHDATA)
            IF(SSPEED(IV) .EQ. 0 .AND. ROUNDABOUT_ID(IL) .NE. 0 .AND. ROUNDABOUT_APPROACH_NUM(IL) .EQ. 0) THEN
              CALL WRITE_VEHICLE_TEXT_STOPPED(THEVEHDATA)
            ENDIF
#endif
            IV = SFOLLOWER(IV)
          ENDDO
...
#else
  CALL WRITE_INCIDENT_TEXT(THELKINCDATA)
#endif
  RETURN
  END
!
...
!
  ...
  USE EVENTS
  IMPLICIT NONE
  INTEGER :: IL, START, DUR, INCTYP, IUP, IDN, LANECODES(N_FREEWAY_LANES)

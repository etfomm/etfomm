  ...
  IF(FLANE(IV) .NE. 0) THEN
    ...
    IF(ILD .NE. 0) THEN
      DO
        ...
        FSPEED(IV) = MAX(FSPEED(IV) + ACCEL * TIMESTEP, 0.)
        IF(ACCEL .GE. -1.0) EXIT
        IF(FSPEED(IV) .LE. FSPEED(ILD)) EXIT
      ENDDO
    ENDIF
    IF(FSPEED(IV) .GT. 0) THEN
      IF(FTHRU_LINK(FLINK(IV)) .NE. 0) THEN
        IF(FSPEED(IV) * TIMESTEP .GE. FLENGTH(FTHRU_LINK(FLINK(IV)))) THEN
          FLOCATION(IV) = -FLENGTH(FTHRU_LINK(FLINK(IV)))
        ENDIF
      ENDIF
      FLEADER(IV) = ILD
      IF(ILD .NE. 0) FFOLLOWER(ILD) = IV
      FENTRYTIME(IV) = SIMTIME
  ...
  IF(SLANE(IV) .NE. 0) THEN
    ...
    IF(ILD .NE. 0) THEN
      DO
        ...
        SSPEED(IV) = MAX(SSPEED(IV) + ACCEL * TIMESTEP, 0.)
        IF(ACCEL .GE. -1.0) EXIT
        IF(SSPEED(IV) .LE. SSPEED(ILD)) EXIT
      ENDDO
    ENDIF
    IF(SSPEED(IV) .GT. 0) THEN
      IF(STHRU_LINK(SLINK(IV)) .NE. 0) THEN
        IF(SSPEED(IV) * TIMESTEP .GE. SLENGTH(STHRU_LINK(SLINK(IV)))) THEN
          SLOCATION(IV) = -SLENGTH(STHRU_LINK(SLINK(IV)))
        ENDIF
      ENDIF
...
! ----------------------------------------------------------------------
  ...
  REAL :: RNDNUM, HEADWAY(5), HMAX, ETIME, EPCT, ACCEL
! ----------------------------------------------------------------------
#ifdef DebugVersion
  integer :: temp
  temp = fid(iv)
...
! --- If the headway is sufficient choose this lane.
      ...
      ELSEIF(FLINK(ILD) .EQ. FTHRU_LINK(IL)) THEN
        ETIME = FENTRYTIME(ILD)
        HEADWAY(I) = SIMTIME - ETIME
        IF(ETIME .GT. SIMTIME) HEADWAY(I) = SIMTIME - ETIME + INITIALIZATION_END
        LEAD(I) = ILD
        ACCEL = 0.0
        CALL FREEWAY_AVOID_COLLISION(IV, ILD, ACCEL)
        IF(ACCEL .LT. 0.) HEADWAY(I) = 0.0
        IF(HEADWAY(I) .GE. MINSEP) THEN
          FLANE(IV) = I
          RETURN
        ENDIF      
      ELSE
...
! --- Do not consider unusable lanes.
      ...
      IF(LEAD(I) .EQ. 0) THEN
        ...
        ELSEIF(FLINK(ILD) .EQ. FTHRU_LINK(IL)) THEN
          ETIME = FENTRYTIME(ILD)
          IF(ETIME .GT. SIMTIME) ETIME = ETIME + INITIALIZATION_END
          HEADWAY(I) = SIMTIME - ETIME
          LEAD(I) = ILD
          ACCEL = 0.
          CALL FREEWAY_AVOID_COLLISION(IV, ILD, ACCEL)
...
! --- Find the last vehicle in the lane.
      ...
      CALL FIND_STREET_LEADER(IV, STHRU_LINK(IL), I, ILD)
      DO WHILE(ILD .NE. 0)
...
! --- Do not choose the lane if there is already a vehicle in it.
      ...
      IF(LEAD(I) .EQ. 0) THEN
        ...
        ELSEIF(SLINK(ILD) .EQ. STHRU_LINK(IL)) THEN
          ...
          ELSE
            ETIME = SENTRYTIME(ILD)
            IF(ETIME .LT. SIMTIME) ETIME = ETIME + INITIALIZATION_END
            HEADWAY(I) = SIMTIME - ETIME
            ACCEL = 0.
            CALL STREET_AVOID_COLLISION(IV, ILD, ACCEL)
            IF(ACCEL .LT. 0.) HEADWAY(I) = 0.0
...
! --- Use freeway fleet distributions if the entry link is a freeway link
! --- and use surface street fleet distributions otherwise.

...
! ----------------------------------------------------------------------
  ...
  integer :: il1, il2, loc1, loc2, usn, dsn 
! ----------------------------------------------------------------------
      ...
      DO ILANE = 1, N_STREET_LANES
        ...
        DO WHILE(IV .NE. 0)
          ...
          IF(SXCODE(IV) .EQ. 0) THEN
            ...
            usn = susn(il)
            dsn = sdsn(il)
            call process_priority(iv, il, il1, il2, loc1, loc2, usn, dsn)
...
! ==================================================================================================
  SUBROUTINE UPDATE_TRAM_VEHICLES
! ----------------------------------------------------------------------
! --- Loop through the tram vehicles and update.
! ----------------------------------------------------------------------
  USE STREET_VEHICLES
  USE STREET_LINKS
  USE BUS_ROUTE_DATA
  ...
  !USE BUS_STATION_DATA
  IMPLICIT NONE
  INTEGER :: NR, INODE, USN, DSN, IV, IL, LOOP_COUNTER
  integer :: il1, il2, loc1, loc2 
! ----------------------------------------------------------------------
#ifdef DebugVersion
  integer :: temp
#endif
  ...
  DO NR = 1, NUMBER_OF_ROUTES
    IF(BUSR_TRAMCARS(NR) .EQ. 0) CYCLE

    DO INODE = BUSR_NNODES(NR) - 1, 1, -1
      USN = BUSR_ROUTE_NODES(NR, INODE)
      DSN = BUSR_ROUTE_NODES(NR, INODE + 1)
      CALL FIND_STREET_LINK(USN, DSN, IL)
      IV = FIRST_VEHICLE(IL, 1)
      LOOP_COUNTER = 0
      DO WHILE(IV .NE. 0)
        LOOP_COUNTER = LOOP_COUNTER + 1
        IF(LOOP_COUNTER .GT. 1000) THEN
          WRITE(MSGTEXT, '(A, I5)') 'UPDATE_TRAM_VEHICLES: INFINITE LOOP, VEHICLE #: ', SID(FIRST_VEHICLE(IL, 1))
          CALL SENDTEXTMSG(M_ERROR)
          EXITFLG = 1
          RETURN
        ENDIF
...
! ==================================================================================================
  subroutine find_approach(il, iact, iap)
! ----------------------------------------------------------------------
! --- Find an approach link number
! ----------------------------------------------------------------------
  ...
  use simparams
  implicit none
  integer, intent(in) :: il, iact
  integer, intent(inout) :: iap
  integer :: i, ilx
! ----------------------------------------------------------------------
  iap = 0
  do i = 1, ac_signals(iact)%n_direct_approaches
    ilx = ac_signals(iact)%direct_approach(i)
    if(susn(ilx) .eq. susn(il) .and. sdsn(ilx) .eq. sdsn(il)) then
      iap = i
      exit
    endif
  enddo
  !if(iap .eq. 0) then
  !  write(msgtext, '(a, i5)') 'FIND_APPROACH::TRAM APPROACH LINK NOT FOUND'
  !  call sendtextmsg(m_error)
  !  write(msgtext, '(a, i4, a, i4)') ' USN = ', susn(il), ' DSN = ', sdsn(il)
  !  call sendtextmsg(m_error)
  !  exitflg = 1
  !endif
  return
...
! ==================================================================================================
  subroutine priority_tsd(iact, il, tphase, iv)
! ----------------------------------------------------------------------
! --- Process TSD
! ----------------------------------------------------------------------
  use simparams
  ...
  use street_links
  implicit none
  integer, intent(in) :: iact, il, tphase, iv
  integer :: ring, phase, indx, policy, iap!, rtmode
! ----------------------------------------------------------------------
  call find_approach(il, iact, iap)
  if(iap .ne. 0) then
    policy = ac_signals(iact)%policy(tphase, iap)
    !rtmode = ac_signals(iact)%rtmode(tphase, iap)
    !WRITE(MSGTEXT, '(A, F10.3))') '  TIME = ', SIMTIME
    !CALL SENDTEXTMSG(M_INFO)
    !WRITE(MSGTEXT, '(A, I4)') '  SIGNAL NODE = ', SDSN(AC_SIGNALS(IACT)%DIRECT_APPROACH(IAP))
    !CALL SENDTEXTMSG(M_INFO)
    !WRITE(MSGTEXT, '(A, I4)') '  VEHICLE ID = ', SID(IV)
    !CALL SENDTEXTMSG(M_INFO)
    !WRITE(MSGTEXT, '(A, F10.3)') '  SPEED = ', SSPEED(IV)
    !CALL SENDTEXTMSG(M_INFO)
    !WRITE(MSGTEXT, '(A, I4)') '  LINK USN = ', SUSN(SLINK(IV))
    !CALL SENDTEXTMSG(M_INFO)
    !WRITE(MSGTEXT, '(A, I4)') '  LINK DSN = ', SDSN(SLINK(IV))
    !CALL SENDTEXTMSG(M_INFO)
    !WRITE(MSGTEXT, '(A, I4)') '  POLICY = ', POLICY
    !CALL SENDTEXTMSG(M_INFO)
    ac_signals(iact)%serving_tram(tphase) = .true.
    if(policy .ne. 0) then
      use_tram_dcs = .true.
      !Delete any master/slave relationship
      ac_signals(iact)%sdp%master_phase = 0
      ac_signals(iact)%sdp%slave_phase = 0
  
      ac_signals(iact)%next_phase = 0
      ac_signals(iact)%checkin_policy(tphase) = policy
      do ring = 1, 2  
        phase = ac_signals(iact)%current_phases(ring)
        if(phase .eq. 0) cycle 
        
        if(phase .eq. tphase) then
          !Check for extension of current phase
          if(policy .eq. 2 .or. policy .eq. 6 .or. policy .eq. 10 .or. policy .eq. 14) then
            if(phase_is_green(iact, tphase)) then
              ac_signals(iact)%extension_flags = ibset(ac_signals(iact)%extension_flags, phase-1)
              !WRITE(MSGTEXT, '(A, I4)') '  SETTING EXTENSION FLAG FOR PHASE = ', PHASE
              !CALL SENDTEXTMSG(M_INFO)
            endif
          endif
        else
          
          !Check for force off of any non-tram phase
          if(policy .eq. 4 .or. policy .eq. 6 .or. policy .eq. 12 .or. policy .eq. 14) then
            do phase = 1, 8
              if(ac_signals(iact)%phase(phase)%in_use) then
                if(.not. ac_signals(iact)%tram_phase(phase)) then
                  ac_signals(iact)%forceoff_flags = ibset(ac_signals(iact)%forceoff_flags, phase-1)
                  !WRITE(MSGTEXT, '(A, I4)') '  SETTING FORCEOFF FLAG FOR PHASE = ', PHASE
                  !CALL SENDTEXTMSG(M_INFO)
                endif
              endif
            enddo
        ...
        if(policy .ge. 8) then
          !Skipping phases
          !Set the reset index to the index of the tram phase
          if(tphase .le. 4) then
            ac_signals(iact)%next_tram_phase(1) = tphase
          else
            ac_signals(iact)%next_tram_phase(2) = tphase
          endif
        endif
        
        !!Process priority return mode
        !write(msgtext, '(a, i4)') '  priority return mode = ', rtmode
        !call sendtextmsg(m_info)
        !if(rtmode .eq. 1) then
        !  ac_signals(iact)%tram_reset_index(ring) = ac_signals(iact)%ring_index(ring)
        !  write(msgtext, '(a, i1, a, i4)') '  ring #', ring, ' setting return phase = ', ac_signals(iact)%current_phases(ring)
        !  call sendtextmsg(m_info)
        !endif
...
! ==================================================================================================
  subroutine priority_extend_green(iact, tphase, iv)
! ----------------------------------------------------------------------
! --- Keep the tram phases green
! ----------------------------------------------------------------------
  use simparams
  use actuated_controllers
  use street_vehicles
  ...
  use street_links
  implicit none
  integer, intent(in) :: iact, tphase, iv
  integer :: ilast, iap, il, policy
! ----------------------------------------------------------------------  
  il = slink(iv)
  call find_approach(il, iact, iap)
  if(iap .ne. 0) then
    policy = ac_signals(iact)%policy(tphase, iap)
    if(policy .eq. 2 .or. policy .eq. 6 .or. policy .eq. 10 .or. policy .eq. 14) then
      ac_signals(iact)%extension_flags = ibset(ac_signals(iact)%extension_flags, tphase-1)
      !Identify the last car in this tram
      ac_signals(iact)%checkin_policy(tphase) = policy
      ilast = iv
      do while(ilast .ne. 0)
        ac_signals(iact)%last_tram_car(tphase) = ilast
        if(last_tram_car(ilast)) exit
        if(.not. tram_link(il) .and. sfleet(iv) .eq. fleet_bus) exit
        ilast = sfollower(ilast)
      enddo
    endif
  endif
  return
...
! --- Determine if the vehicle should make a lane change.
    ...
    IF(.NOT. TRAM_LINK(SLINK(IV))) THEN
...
! --- Limit the acceleration to achieve a safe following distance.
    ...
    IF(FLEADER(IV) .NE. 0) THEN
      CALL FREEWAY_CAR_FOLLOW(IV, FLEADER(IV), ACCEL, DISTANCE)
    ENDIF
...
! --- If the vehicle is cooperating with an EV make it stay behind the EV.
    ...
    IF(FPSEUDO_LEADER(IV) .NE. 0) THEN
      IF(DISTANCE_TO_SEGMENT_END(FPSEUDO_LEADER(IV)) .LT. DISTANCE_TO_SEGMENT_END(IV)) THEN
        CALL FREEWAY_CAR_FOLLOW(IV, FPSEUDO_LEADER(IV), ACCEL, DISTANCE)
      ENDIF
    ENDIF
...
! ----------------------------------------------------------------------
  ...
  INTEGER :: ILN, ILD, IL1, IL2, ISPD, ISTAT, IEVENT, IFRSPD, ILANE, IEV, ITURN, IREC
  LOGICAL :: WSTOP, CHECK, IGNORE
...
! ----------------------------------------------------------------------
  ...
  LOC1 = SLOCATION(IV)
  IF(TRAM_LEADER(IV) .NE. 0) THEN
    IL1 = SLINK(IV)
    ILD = TRAM_LEADER(IV)
    SACCELERATION(IV) = SACCELERATION(ILD)
    SSPEED(IV) = SSPEED(ILD)
    TRAVEL = DISTANCE_TRAVELED(ILD)
    SLOCATION(IV) = SLOCATION(IV) + TRAVEL
    IF(SLOCATION(IV) .GT. SLENGTH(SLINK(IV))) THEN
      IF(IV .EQ. SLAST_VEHICLE(SLINK(IV), SLANE(IV))) SLAST_VEHICLE(SLINK(IV), SLANE(IV)) = 0
      SLAST_VEHICLE_OUT(IL1, SLANE(IV)) = IV
      SLOCATION(IV) = SLOCATION(IV) - SLENGTH(SLINK(IV)) 
      FIRST_VEHICLE(SLINK(IV), 1) = SFOLLOWER(IV)
      if(sfollower(iv) .ne. 0) then
        if(tram_leader(sfollower(iv)) .eq. 0) then
      ...
      ELSEIF(ITURN .EQ. TC_LDIAG) THEN
        IREC = LEFT_DIAG_LINK(IL1)
      ELSEIF(ITURN .EQ. TC_RDIAG) THEN
        IREC = RIGHT_DIAG_LINK(IL1)
      ENDIF 
      IF(IREC .NE. 0) THEN
        SLINK(IV) = IREC
      ELSE
        WRITE(MSGTEXT, '(A,I4)') '  Link was not found on tram route # ', SROUTEID(IV)
        CALL SENDTEXTMSG(M_ERROR)
        WRITE(MSGTEXT, '(A,I4, A, I4)') '   Current Link ', SUSN(IL1), '->', SDSN(IL1)
        CALL SENDTEXTMSG(M_ERROR)
        EXITFLG = 1
        RETURN
      ENDIF
      ...
      IF(ILANE .NE. 0) THEN
        IF(BLOCKAGE(IL1, ILANE) .NE. 0) THEN
          ...
          IF(SLEADER(IV) .EQ. 0) THEN
            IF(SLOCATION(IV) - SVLENGTH(IV) .LT. EVENT_LOCATION(IEVENT)) CHECK = .TRUE.
          ELSE
            IF(SLOCATION(SLEADER(IV)) .GT. EVENT_LOCATION(IEVENT)) THEN
              IF(SLOCATION(IV) - SVLENGTH(IV) .LT. EVENT_LOCATION(IEVENT)) CHECK = .TRUE.
            ENDIF
          ENDIF
          IF(CHECK) THEN
            DIST = EVENT_LOCATION(IEVENT) - SLOCATION(IV)
            CALL CHECK_BLOCKAGE(IV, INT(DIST), DECEL, WSTOP)
            IF((DECEL .LT. -8.0 .OR. DIST .LT. 0) .AND. SIMTIME .EQ. EVENT_BEGIN_TIME(IEVENT)) THEN
              DO IEV = 1, NUMBER_OF_EVENTS
                IF(EVENT_GROUP_ID(IEV) .EQ. EVENT_GROUP_ID(IEVENT)) THEN
                  IF(EVENT_BEGIN_TIME(IEV) .GE. EVENT_BEGIN_TIME(IEVENT)) THEN
                    EVENT_BEGIN_TIME(IEV) = EVENT_BEGIN_TIME(IEV) + TIMESTEP
                    EVENT_END_TIME(IEV) = EVENT_END_TIME(IEV) + TIMESTEP
                  ENDIF
                ENDIF
              ENDDO
            ELSE
              IF((EVENT_LOCATION(IEVENT) .GE. SLENGTH(IL1) - CROSS_WALK(IL1)) .AND. &
                (SSPEED(IV) .LE. STOP_SPD .AND. DIST .GT. 5)) THEN
                ACCEL = MIN(ACCEL, DIST / 3.)
              ELSE
  ...
  ! --- try to enter the station.
    ...
    ! --- When the timer expires try to move from the station.
            ...
            IF(DWELL_TIMER(IV) .LE. 0.0) THEN
              IF(SLEADER(IV) .EQ. 0) THEN
                CALL DISCHARGE_FROM_STATION(IV, ACCEL, ISTAT)
              ELSEIF(SLOCATION(SLEADER(IV)) - SVLENGTH(SLEADER(IV)) .GT. 10) THEN
                CALL DISCHARGE_FROM_STATION(IV, ACCEL, ISTAT)
              ENDIF
            ELSE
              ...
              ELSE
                ACCEL = 0.
              ENDIF
              CALL UPDATE_STREET_STATS(IV, IL1, 0, TRAVEL, 0., TIMESTEP, 0.)
            ENDIF
          ELSE
            IF(ISTAT .NE. 0) THEN
              IF(IL1 .EQ. BUS_STATION_LIST(ISTAT)%LINK) THEN
                IF(.NOT. WILL_BYPASS(IV)) THEN
                  IF(BUS_STATION_LIST(ISTAT)%LOCATION - SLOCATION(IV) .LT. 250) THEN
                    CALL STREET_RANDOM(SSEED, RNDNUM)
                    IF(RNDNUM .GT. BUS_STATION_LIST(ISTAT)%BYPASS_PCT) THEN
                      CALL APPROACH_BUS_STATION(IV, ACCEL)
                      TRAVEL = SLOCATION(IV) - LOC1
                    ELSE
                      WILL_BYPASS(IV) = .TRUE.
                    ENDIF
                  ENDIF
                ENDIF
                IF(WILL_BYPASS(IV)) THEN
                  !If the bus or tram will bypass the station accumulate data for the MOEs
                  !when the vehicle passes the station
                  IF(SLOCATION(IV) .GE. BUS_STATION_LIST(ISTAT)%LOCATION - BUS_STATION_LIST(ISTAT)%CAPACITY * SVLENGTH(IV)) THEN
                    IF(.NOT. INITMODE) THEN
                      BUS_STATION_LIST(ISTAT)%COUNT = BUS_STATION_LIST(ISTAT)%COUNT + 1
                      BUS_STATION_LIST(ISTAT)%TIME_FROM_LAST = BUS_STATION_LIST(ISTAT)%TIME_FROM_LAST + STATION_TRAVELTIME(IV)
                    ENDIF
                    STATION_TRAVELTIME(IV) = 0.
                    NEXT_STOP(IV) = NEXT_STOP(IV) + 1
                    WILL_BYPASS(IV) = .FALSE.
                  ENDIF
                ENDIF
              ENDIF
            ENDIF
  ...
  ! --- determine if it needs to slow down.
        ...
        IF(ILANE .NE. 0) THEN
          IF(SLANECODES(IV, ILANE) .NE. LC_GOOD) THEN
            DIST = SLENGTH(IL1) - SLOCATION(IV) - CROSS_WALK(IL1)
        ...
        IF(ILD .EQ. 0) THEN
          ...
          ISTOPL = SLENGTH(IL1) - SLOCATION(IV) - CROSS_WALK(IL1)
  ...
  ! --- Apply the resulting acceleration.
        ...
        IF(SLINK(IV) .NE. 0) THEN
          ...
          IF(WSTOP) THEN
            ...
            SSPEED(IV) = 0.
            ILOC = SLOCATION(IV)
            TRAVEL = MAX(SLOCATION(IV), FLOAT(SLENGTH(SLINK(IV)) - CROSS_WALK(SLINK(IV)))) - ILOC
...
! --- Determine if the vehicle has passed a detector.
    ...
    ENDIF
  ENDIF
  IF(QSTATE(IV) .NE. QS_DWELL) STATION_TRAVELTIME(IV) = STATION_TRAVELTIME(IV) + TIMESTEP
  RETURN
  END
...
10 CONTINUE
  ...
  COVERED = .FALSE.
!delete the following lines if need to genrate ETEDItor anamation. The line is for  tsd file  ONLY
#define TSIS_COMPATIBLE  
#ifdef TSIS_COMPATIBLE
  ILANE1 = SDETECTOR(IDET)%LANE1
  ILANE2 = SDETECTOR(IDET)%LANE2
  ...
  ENDIF
#else
  DO ILANE1 = 1, TOTAL_LANES(IL)
    IF(SDETECTOR(IDET)%DETECTOR_LANES(ILANE1) .EQ. ILN) THEN
      COVERED = .TRUE.
      EXIT
    ENDIF
  ENDDO
...
! --- limit the target speed to the turning speed for this link.
  ...
  IF(STURNCODE(IV) .EQ. TC_LEFT .OR. STURNCODE(IV) .EQ. TC_RIGHT) THEN
    IF(SLENGTH(IL) - SLOCATION(IV) .LE. RSPD ** 2 / 7) THEN
      DIST = SLENGTH(IL) - SLOCATION(IV) - SSPEED(IV) * TIMESTEP
      IF(DIST .GT. 0.0) THEN
        IF(STURNCODE(IV) .EQ. TC_LEFT) THEN
          RSPD = SQRT(8 * DIST + LT_SPEED(IL) ** 2)
          IF(SLANE(IV) .NE. 0) THEN
            IF(SLANECODES(IV, SLANE(IV)) .NE. LC_GOOD .AND. DIST .LT. 50.) THEN
              RSPD = MIN(RSPD, LT_SPEED(IL) / 2.)
            ENDIF
          ENDIF
...
! --- limit the target speed to the turning speed of the previous link.
  ...
  IF(ARC_LENGTH(IV) .NE. 0 .AND. ARC_LOCATION(IV) .LT. LIMITED_SPEED_DIST(IV)) THEN
    ...
    ELSEIF(PREV_TURNCODE(IV) .EQ. TC_RIGHT) THEN
      ...
      ENDIF
    ELSEIF(PREV_TURNCODE(IV) .EQ. TC_LEFT) THEN
      IF(ROUNDABOUT_ID(SPREVLINK(IV)) .EQ. 0) THEN
        RSPD = LT_SPEED(SPREVLINK(IV))
      ENDIF
    ENDIF
  ENDIF
...
! --- If the vehicle will exit farther downstream use a diagonal turn code.
              ...
              ELSE
                ...
                IF(FLANECODES(IV, FLANE(IV)) .EQ. LC_VACATE) THEN
                  IF(REMAINING_DIST(IV) .EQ. 0.) THEN
                    REMAINING_DIST(IV) = OFFRAMP_WARN_DISTANCE(OBJECT_LIST(IOBJ)%LINK)
                  ELSE
                    REMAINING_DIST(IV) = MIN(REMAINING_DIST(IV), FLOAT(OFFRAMP_WARN_DISTANCE(OBJECT_LIST(IOBJ)%LINK)))
                  ENDIF
                ENDIF
              ENDIF   
            ENDIF
...
! --- Determine if congestion exists.
              ...
              IF(FTHRU_LINK(ILON) .NE. 0) THEN
                IF(AVERAGE_SPEED(ILON) .LE. ANTICIP_WARNING_SPEED(FTHRU_LINK(ILON))) THEN
...
! --- currently does not have a leader.
                ...
                IF(SEGMENT(IRL) .NE. ISEGMENT(IV)) THEN
                  ...
                  ENDIF
                  IF(ILD .NE. 0) CALL FREEWAY_CAR_FOLLOW(IV, ILD, ACCEL, DISTANCE)
...
! --- to car-follow a vehicle on the surface street.
            ...
            ENDIF
            IF(INODE .NE. 0) THEN
! Tom suggsted to delete sections below to avoid suddent slow down before interface node
!              IF(FCROSS1(INODE, FLANE(IV)) .NE. 0) THEN
!                CALL FREEWAY_CAR_FOLLOW(IV, FCROSS1(INODE, FLANE(IV)), ACCEL, DISTANCE)
!              ELSE++
...
! --- to avoid entering the HOV lane make it slow down to allow a lane change.
          ...
          ELSEIF(OBJECT_LIST(IOBJ)%ITYPE .EQ. M_MERGE_POINT) THEN
            FDESIREDSPEED(IV) = MIN(FDESIREDSPEED(IV), FLOAT(AVERAGE_SPEED(FTHRU_LINK(IL))))
            ISEG = SEGMENT(FTHRU_LINK(IL))
...
! --- that leads to a mainline link, or a full auxiliary that exits the network.
      
      WILL_EXIT = .FALSE.
      IF(FTHRU_LINK(IL) .EQ. 0) THEN
        WILL_EXIT = .TRUE.
      ELSE
        IF(NODE_TYPE(FDSN(FTHRU_LINK(IL))) .NE. NT_INTERN) WILL_EXIT = .TRUE.
      ENDIF
      IF(will_exit) THEN
        IF(OBJECT_LIST(IOBJ)%VALUE .EQ. 3) FLANECODES(IV, OBJECT_LIST(IOBJ)%LANE) = LC_GOOD
      ELSEIF(RECEIVING_LANE(IL, OBJECT_LIST(IOBJ)%LANE) .NE. 0) THEN
...
! ----------------------------------------------------------------------
  ...
  INTEGER :: ITYPE, NBR, IPX, IVN, IRL, ILN, IVX
...
! --- If the vehicle has a leader but the leader is not on the off-ramp ignore that leader.
              ...
              ILD = FLEADER(IV)
              IF(ILD .NE. 0) THEN
                IF(FLINK(ILD) .NE. FLINK(IV)) THEN
                  FFOLLOWER(ILD) = FFOLLOWER(IV)
                  IF(FFOLLOWER(IV) .NE. 0) THEN
                    FLEADER(FFOLLOWER(IV)) = ILD
                  ENDIF
                  ILD = 0
                ENDIF
              ENDIF
          ...
          ELSE
            IF(NODE_TYPE(FDSN(FTHRU_LINK(IL))) .EQ. NT_EXTERN) THEN
              IF(FFLEET(IV) .EQ. FLEET_BUS .AND. .NOT. INITMODE) THEN
                NBR = FROUTEID(IV)
                BUSR_TRIPS(NBR) = BUSR_TRIPS(NBR) + 1
                BUSR_PERSONTRIPS(NBR) = BUSR_PERSONTRIPS(NBR) + AVG_OCCS(FVTYPE(IV))
              ENDIF
...
! Tom disabled four lines 09/11/2024
              FLOCATION(IV) = FLOCATION(IV) + FLENGTH(FLINK(IV))
              ACCEL = 0.
              CALL FREEWAY_CAR_FOLLOW(IVN, IV, ACCEL, DISTANCE)
...
! --- Increment the number of vehicles entering the new link.
      ...
      IF(.NOT. INITMODE) FENTERING(IL) = FENTERING(IL) + 1
...
! ----------------------------------------------------------------------
  ...
  INTEGER :: ITYPE, NBR, ITURN, IREC, IACT, PHASE, INTCH, IAP
  REAL :: RNDNUM
! ----------------------------------------------------------------------
#ifdef DebugVersion
  integer :: temp
  ...
  IF(IACT .NE. 0) THEN
    ...
    ENDIF
    if(phase .ne. 0) then
      !If a tram enters the link tell the signal to hold green
      !The flag will be cleared when the last car crosses the checkout detector
      if(tram_link(il) .and. iact .ne. 0) then
        call find_approach(il, iact, iap)
        if(iap .ne. 0) then
          if(ac_signals(iact)%policy(phase, iap) .ne. 0) then
            ac_signals(iact)%hold_for_tram = .true.
          endif
        endif
      endif
    ENDIF                
...
! --- Increment the number of vehicles discharged from the prior link.
  ...
  IF(.NOT. INITMODE) THEN
    ...
    IF(IACT .NE. 0 .AND. PHASE .NE. 0) THEN
      ...
      IF(IREC .NE. 0) THEN
        IF(NODE_TYPE(SDSN(IREC)) .EQ. NT_EXTERN) THEN
          IF(SFLEET(IV) .EQ. FLEET_BUS .AND. .NOT. INITMODE) THEN
            NBR = SROUTEID(IV)
            IF(.NOT. TRAM_FOLLOWER(IV)) THEN
              BUSR_TRIPS(NBR) = BUSR_TRIPS(NBR) + 1
              IF(BUSR_TRAMCARS(NBR) .NE. 0) THEN
                BUSR_PERSONTRIPS(NBR) = BUSR_PERSONTRIPS(NBR) + BUSR_TRAMCARS(NBR) * AVG_OCCS(SVTYPE(IV))
              ELSE
                BUSR_PERSONTRIPS(NBR) = BUSR_PERSONTRIPS(NBR) + AVG_OCCS(SVTYPE(IV))
              ENDIF
            ENDIF
          ENDIF
        ENDIF
...
! --- Increment the number of vehicles entering the new link.
      ...
      IF(.NOT. INITMODE) SENTERING(IL) = SENTERING(IL) + 1
...
! --- it has no leader.
      ...
      IF(.NOT. TRAM_LINK(IL) .AND. SFOLLOWER(IV) .NE. 0) THEN
...
! --- The vehicle is still approaching the stopbar.
        ...
        !Limit the vehicle speed to 5 mph.
        IF(FSPEED(IV) .GT. 7.3) THEN
          ACCEL = -(FSPEED(IV) ** 2) / (2 * D)
          ACCEL = MIN(A, ACCEL)
        ELSEIF(FSPEED(IV) + ACCEL * TIMESTEP .GT. 7.3) THEN
          ACCEL = MIN(ACCEL, (7.3 - FSPEED(IV)) / TIMESTEP)
        ENDIF
      ELSE
...
! ----------------------------------------------------------------------
  ...
  IMPLICIT NONE
  INTEGER, INTENT(IN) :: IV
  INTEGER :: IL, TCODE, IN, N1, N2, NL, ICODE
  REAL :: RNDNUM
! ----------------------------------------------------------------------
#ifdef DebugVersion
  integer :: temp
        ...
        ELSE
          ...
          IF(ICODE .EQ. -1) THEN
            IF(TRAM_LINK(IL)) THEN
              WRITE(MSGTEXT, '(A,I4)') '  Link was not found on tram route # ', SROUTEID(IV)
            ELSE
              WRITE(MSGTEXT, '(A,I4)') '  Link was not found on bus route # ', SROUTEID(IV)
            ENDIF
            CALL SENDTEXTMSG(M_ERROR)
            WRITE(MSGTEXT, '(A,I4, A, I4)') '   Current Link ', SUSN(IL), '->', SDSN(IL)
            CALL SENDTEXTMSG(M_ERROR)
            WRITE(MSGTEXT, '(A,I4, A, I4)') '   Next Link ', N1, '->', N2
            CALL SENDTEXTMSG(M_ERROR)
            EXITFLG = 1
            RETURN
          ELSE
  ...
  ELSEIF(IN_TURNING_WAY(IV)) THEN
    ...
    ELSEIF(ICODE .EQ. S_STOP) THEN
      ...
      ELSEIF(SSPEED(IV) .GT. STOP_SPD) THEN
        WPERM = .FALSE.
      ELSEIF(SLOCATION(IV) .LT. SLENGTH(IL) - CROSS_WALK(IL)) THEN
        WPERM = .FALSE.
      ELSE
        WPERM = .TRUE.
      ENDIF
...
! --- Determine if current signal state allows left turn.
        ...
        IF(NL .NE. S_RED .AND. AMBER .AND. FTC_SIGNALS(ISIG)%TIME_IN_INTERVAL .LE. 1.0) THEN
...
! --- Determine if current signal state allows right turn.
        ...
        IF(.NOT. WPERM .AND. RTOR(IL) .AND. SSPEED(IV) .EQ. 0 .AND. &
           SLOCATION(IV) .GE. SLENGTH(IL) - 3 - CROSS_WALK(IL)) THEN
          IF(.NOT. RTOR_BLOCKAGE(IL)) THEN
            WPERM = .TRUE.
            RTOR_FLAG(IV) = .TRUE.
          ENDIF
        ENDIF
...
! --- Compute the required stopping deceleration if the vehicle must stop at the end of the link.
  ...
  ELSE
    DIST = SLENGTH(IL) - SLOCATION(IV) - CROSS_WALK(IL)
  ENDIF
  STOPDIST = -(SSPEED(IV) ** 2) / (2 * RMIN)
...
! ----------------------------------------------------------------------
  ...
  INTEGER :: ITURN, IL, IRLANE, IRL, ILD, ILANE, ILN, IVN, SFL, IFL, NBR
...
! --- Decrement the vehicle's discharge timer.
      ...
      !Decrement the discharge timer for the vehicles in queue behind this vehicle.
      !need to have a logic to break the infinit loop
      IFL = SFOLLOWER(IV)
      DO WHILE(IFL .NE. 0)
        SDISCH_TIMER(IFL) = SDISCH_TIMER(IFL) - TIMESTEP
        IFL = SFOLLOWER(IFL)
      ENDDO
          ...
          IF(IRL .NE. 0) THEN
            IF(BLOCKAGE(IRL, IRLANE) .NE. 0) THEN
              ...
              IF(SIMTIME .GE. EVENT_BEGIN_TIME(IEVENT) .AND. SIMTIME .LE. EVENT_END_TIME(IEVENT)) THEN
                IF(ITURN .EQ. TC_RIGHT) THEN
                  DIST = SLENGTH(IL) - SLOCATION(IV) + EVENT_LOCATION(IEVENT) - UP_INT_WIDTH(IRL)
                ELSEIF(ITURN .EQ. TC_LEFT) THEN
                  DIST = SLENGTH(IL) - SLOCATION(IV) + EVENT_LOCATION(IEVENT) - 0.5 * UP_INT_WIDTH(IRL)
                ELSE
                  DIST = SLENGTH(IL) - SLOCATION(IV) + EVENT_LOCATION(IEVENT)
                ENDIF
                IF(DIST .GT. 0 .AND. DIST .LT. UP_INT_WIDTH(IRL) + SSPEED(IV) * TIMESTEP) THEN
                  CALL CHECK_BLOCKAGE(IV, DIST, DECEL, WSTOP)
...
! --- If the vehicle had a follower on the previous link, clear it.
        ...
        IF(.NOT. TRAM_LINK(IL)) SFOLLOWER(IV) = 0
        IF(NODE_TYPE(SDSN(IRL)) .EQ. NT_EXTERN) THEN
    ...
    ! --- Determine if the last vehicle in each lane will block the discharging vehicle.
        IF(SLOCATION(IVX) - SVLENGTH(IVX) .LT. BLOCKDIST) THEN
          IF(SPREVLINK(IVX) .EQ. SLINK(IV) .AND. SPREVLANE(IVX) .GT. SLANE(IV)) THEN
            CYCLE
          ELSEIF(PREV_TURNCODE(IVX) .EQ. TC_RIGHT) THEN
            CYCLE
          ELSE
            WPERM = .FALSE.
            EXIT
      ...
      IF(IL .NE. 0) THEN
        ...
        IF(IBR .NE. 0) THEN
          IF(.NOT. TRAM_FOLLOWER(IV)) THEN
            ...
            BUSR_TRAVELTIME(IBR) = BUSR_TRAVELTIME(IBR) + TRAVEL_TIME
            BUSR_TRAVELDIST(IBR) = BUSR_TRAVELDIST(IBR) + TRAVEL_DIST
          ENDIF
        ENDIF
...
! ----------------------------------------------------------------------
  ...
  REAL :: ISTOP, DIST, AVGLEN = -1, RMAX, DECEL
  LOGICAL :: FULL
! ----------------------------------------------------------------------
#ifdef DebugVersion
  integer :: temp
...
! --- and stop when it is far enough downstream.
  ...
  ELSE
    ISTOP = BUS_STATION_LIST(ISTAT)%LOCATION
    DIST = ISTOP - SLOCATION(IV)
    IF(DIST .LT. (SSPEED(IV) ** 2) / 7 .AND. SSPEED(IV) .GT. STOP_SPD) THEN
      DECEL = -SSPEED(IV) ** 2 / (2 * DIST)
      !Account for lag time
      IF(DIST .LT. SSPEED(IV) * TIMESTEP .AND. SSPEED(IV) .GT. DIST .AND. SACCELERATION(IV) .GT. 0) THEN
        DECEL = 1.5 * DECEL
      ENDIF
      ACCEL = MIN(ACCEL, DECEL)
      ACCEL = MAX(ACCEL, RMAX)
...
! --- Determine the specific dwell time for a bus entering a station.
  ...
  NBR = SROUTEID(IV)
  BUSR_DWELLTIME(NBR) = BUSR_DWELLTIME(NBR) + DWELL_TIMER(IV) 
  RETURN
  END
...
! --- in the station.
    ...
    ELSE
      BUS_STATION_LIST(ISTAT)%FRONT = 0
    ENDIF
    NEXT_STOP(IV) = NEXT_STOP(IV) + 1
...
! ==================================================================================================
  SUBROUTINE CHECK_BLOCKAGE(IV, DIST, DECEL, WSTOP)
! ----------------------------------------------------------------------
...
! --- If the vehicle is in the correct lane and segment, select it as the putative leader.
...
! --- Do not allow the vehicle to use the lane unless it is the leftmost through lane.
      !or the lane to the left is left turn only
                 
            IF(I .LT. LAST_FULL_LANE(SLINK(IV))) THEN
              ICODE = CHANNELIZATION(NLINK, I+1)
              IF(ICODE .NE. 1) THEN
                SLANECODES(IV, I) = LC_VACATE
              ENDIF
            ENDIF
...
! --- Do not allow the vehicle to use the lane unless it is the rightmost through lane.
      !or the lane to the right is right turn only
                 
            IF(I .GT. FIRST_FULL_LANE(SLINK(IV))) THEN
              ICODE = CHANNELIZATION(NLINK, I-1)
              IF(ICODE .NE. 4) THEN
                SLANECODES(IV, I) = LC_VACATE
              ENDIF
...
! --- the current link keep it in the rightmost full lane.
    ...
    ENDIF
    IF(WSTOP) THEN
      DO I = FIRST_FULL_LANE(NLINK) + 1, TOTAL_LANES(NLINK)
...
! --- if they have a receiving lane on the off-ramp receiving link.
    ...
    IF(NLINK .EQ. DESTINATION(IV) .OR. OFFRAMP_LINK(NLINK) .EQ. DESTINATION(IV)) THEN
      NLANES = 0
      DO I = 1, N_FREEWAY_LANES
        IF(CURRENT(I) .NE. LC_NULL) THEN
          IF(EXIT_LANE(ILINK, I) .NE. 0) THEN
            FLANECODES(IV, EXIT_LANE(ILINK, I)) = LC_VACATE
            IF(RECEIVING_LANE(NLINK, EXIT_LANE(ILINK, I)) .NE. 0) THEN
              FLANECODES(IV, EXIT_LANE(ILINK, I)) = LC_GOOD
              NLANES = NLANES + 1
            ENDIF
          ENDIF
        ENDIF
...
! ----------------------------------------------------------------------
  ...
  ELSE
    ...
    IF(DIST .GT. 0.) THEN
      ...
      IF(TRAVEL .GT. 0.) THEN
        IF(FRAC .GE. 1.0) THEN
          IF(SLOCATION(IV) .LT. 0.) THEN
            !Vehicle is just entering the arc, but will travel beyond the end of the arc in this timestep.
  ...
  IF(FLANE(IV) .EQ. MERGING_LANE(RAMP)) THEN
    ...
    IF(ILD .NE. 0) THEN
      IF(FLINK(ILD) .EQ. JLINK) THEN
        CALL FREEWAY_CAR_FOLLOW(IV, ILD, ACCEL, DISTANCE)
      ENDIF
    ENDIF
  ENDIF
...
! ==================================================================================================
  subroutine process_priority(iv, il, il1, il2, loc1, loc2, usn, dsn)
  use street_vehicles
  use street_links
  use actuated_controllers
  use street_detectors
  ...
  use node_table
  implicit none
  integer, intent(in) :: iv, il, il1, il2, loc1, loc2, usn, dsn
  integer :: ilx, iact, ring, phase, sigdist, tphase, indx
  integer :: projected_distance, checkin_distance, ilast, iap
  integer :: detloc, ndet
  logical :: other_index, checked_in, force_off
! ----------------------------------------------------------------------
#ifdef DebugVersion
  integer :: temp
  temp = sid(iv)
  ...
  if((tram_link(il) .and. tram_leader(iv) .eq. 0) .or. sfleet(iv) .eq. fleet_bus) then
    !This is the lead car of a tram or a bus
    checked_in = .false.
    if(il1 .eq. il2) then
      if(sfirst_transit_detector(il1) .ne. 0) then
        ndet = sfirst_transit_detector(il1) 
        detloc = transit_detector(ndet)%location
        !Determine if the tram crossed the detector during the current time step
        if((detloc .eq. 0 .and. loc1 .le. 0 .and. loc2 .gt. 0) .or. (loc2 .ge. detloc .and. loc1 .lt. detloc)) then
          iact = transit_detector(ndet)%signal_node
          tphase = transit_detector(ndet)%associated_phase
          call priority_tsd(iact, il, tphase, iv)
          checked_in = .true.
          !Identify the last car in this tram
          ilast = iv
          do while(ilast .ne. 0)
        ...
        ndet = transit_detector(ndet)%next_det
        do while(ndet .ne. 0)
          detloc = transit_detector(ndet)%location
          if((detloc .eq. 0 .and. loc1 .eq. 0 .and. loc2 .gt. 0) .or. (loc2 .ge. detloc .and. loc1 .lt. detloc)) then
            iact = transit_detector(ndet)%signal_node
            tphase = transit_detector(ndet)%associated_phase
            call priority_tsd(iact, il, tphase, iv)
            checked_in = .true.
      ...
      !Check for another tram approaching within 10 seconds
      if(.not. checked_in) then
        iact = ac_signal_id(il)
        if(iact .ne. 0) then
          do ring = 1, 2
            phase = ac_signals(iact)%current_phases(ring)
            if(phase .eq. 0) cycle
            if(ac_signals(iact)%tram_phase(phase)) then
              sigdist = slength(il) - loc2
              if(sigdist .le. 10 * sspeed(iv)) then
                call priority_extend_green(iact, phase, iv)
              endif
            endif
          enddo
        endif
      ...
      if(sfirst_transit_detector(il1) .ne. 0) then
        ndet = sfirst_transit_detector(il1) 
        detloc = transit_detector(ndet)%location
        if(loc1 .lt. detloc) then
          iact = transit_detector(ndet)%signal_node
          tphase = transit_detector(ndet)%associated_phase
          call priority_tsd(iact, transit_detector(ndet)%link, tphase, iv)
          !Identify the last car in this tram
          ilast = iv
          do while(ilast .ne. 0)
            ac_signals(iact)%last_tram_car(tphase) = ilast
      ...
      if(il2 .ne. 0) then
        if(sfirst_transit_detector(il2) .ne. 0) then
          ndet = sfirst_transit_detector(il2) 
          detloc = transit_detector(ndet)%location
          if(loc2 .ge. detloc) then
            iact = transit_detector(ndet)%signal_node
            tphase = transit_detector(ndet)%associated_phase
            call priority_tsd(iact, transit_detector(ndet)%link, tphase, iv)
            !Identify the last car in this tram
  ...
  if(node_type(usn) .eq. nt_intern .and. node_type(dsn) .eq. nt_intern) then
    if((last_tram_car(iv) .or. sfleet(iv) .eq. fleet_bus) .and. .not. last_car_checked_out(iv)) then
      if(nact(susn(il)) .ne. 0 .and. sprevlink(iv) .ne. 0) then
        iact = nact(susn(il))
        tphase = priority_link_phase(sprevlink(iv))
        if(iact .ne. 0 .and. tphase .ne. 0) then
          if(iv .eq. ac_signals(iact)%last_tram_car(tphase)) then
            call find_approach(sprevlink(iv), iact, iap)
            if(iap .ne. 0) then
              if(slocation(iv) - svlength(iv) .gt. ac_signals(iact)%checkout_distance(tphase, iap)) then
                last_car_checked_out(iv) = .true.
                ac_signals(iact)%last_tram_car(tphase) = 0
                ac_signals(iact)%hold_for_tram = .false.
                if(tphase .ne. 0) then
                  !Allow the phase to terminate normally
                  ac_signals(iact)%extension_flags = ibclr(ac_signals(iact)%extension_flags, tphase-1)
                  if(tphase .le. 4) then
                    ac_signals(iact)%next_tram_phase(1) = 0
                  else
                    ac_signals(iact)%next_tram_phase(2) = 0
                  endif
                  !Terminate the phase if it is tram-only
                  if(iv .eq. ac_signals(iact)%last_tram_car(tphase)) then
                    if(ac_signals(iact)%tram_only_phase(tphase)) then
                      force_off = .true.
                      if(sfollower(iv) .ne. 0) then
                        if(slink(sfollower(iv)) .eq. slink(iv)) then
                          force_off = .false.
                          ac_signals(iact)%last_tram_car(tphase) = sfollower(iv)
                          last_tram_car(iv) = .false.
                          last_tram_car(sfollower(iv)) = .true.
                        endif
                      endif
                    endif
                    if(force_off) then
                      ac_signals(iact)%forceoff_flags = ibset(ac_signals(iact)%forceoff_flags, tphase - 1)
                    endif
                  endif
                endif
              endif

  ...
  ! --- as scheduled. Also check for delayed vehicles in the queue.
      ...
      ELSEIF(EXITFLG .GT. 0) THEN
        IF(EXITFLG .EQ. 1) THEN
          WRITE(MSGTEXT, '(A)') 'SIMULATION FAILED'
        ELSEIF(EXITFLG .EQ. 2) THEN
          WRITE(MSGTEXT, '(A)') 'SIMULATION ABORTED BY USER'
        ENDIF
        CALL SENDTEXTMSG(M_INFO)
        RUNTOEQUILIBRIUM = EXITFLG
        EXIT
      ENDIF
    ENDDO
    SIMTIME = 0.
...
! ----------------------------------------------------------------------
  ...
  ! --- in the entry queues.
    ...
    IF(MOD(SIMTIME, 60.) .LT. TIMESTEP/2 .AND. SIMTIME .LT. SIMULATION_END) THEN
      CALL TRIPGENERATOR
  !LZ added a function to export entry vehicle departure time    
  !    CALL ExportEntryVehicles
  ...
  ! --- Run the simulation for one time step.
    ...
    ELSEIF(ERROR_FLAG .NE. 0 .OR. ERROR_COUNT .NE. 0) THEN
      SIMULATE = 1
    ELSEIF(EXITFLG .GT. 0) THEN
      SIMULATE = EXITFLG
      IF(EXITFLG .EQ. 1) THEN
...
!=======================================================================
  ...
  INTEGER :: NV, IV, USN, DSN, TEMP, TP, NVEHLINK, NMATCHZ, IOSTAT
  INTEGER :: NMETER, IL, ITIME, LAST, FIRST_INC_TIME, LAST_INC_TIME
  REAL :: INCTIME, TIME1, TIME2
  LOGICAL :: HAS_INCIDENTS = .FALSE.
   ! Two varables added by LZ to check if there is a accident file before read the file 
...
! ----------------------------------------------------------------------
  ...
  ! Checked if the accident file exsited first LZ 08/02/2024
  
  file_name = TRIM(LINFNAME(1:IROOT-1))//'_INCIDENT_DATA.TXT'  
  INQUIRE(FILE=file_name, EXIST=ios)
  if (ios .NE. 0) THEN

  OPEN(5, FILE = LINFNAME(1:IROOT-1)//'_INCIDENT_DATA.TXT', STATUS='OLD', ERR=5)
  HAS_INCIDENTS = .TRUE.
  READ(5, *) FIRST_INC_TIME, THELKINCDATA
  DO WHILE(.TRUE.)
    READ(5, *, END=5) LAST_INC_TIME, THELKINCDATA
  ENDDO
5 CONTINUE  
  REWIND(5)
  ENDIF
...
11 CONTINUE  
  CALL SIMSTATUS(SIM_BEGINSIMULATION)
  TP = 1
  !Added by LZ to flag number of vehicles in link data does not macth actually in VEHICLE data, initially set to 1
  NMATCHZ = 1
  ! Added by LZ for recording processing 
  NVEHLINK = 0
...
21      CONTINUE 
        IF (NMATCHZ .EQ. 1) THEN
            READ(2, *, IOSTAT=IOSTAT, ERR=40, IOMSG=ETEXT) TIME2, USN, DSN, &
            ...
            IF (IOSTAT /= 0) THEN
                ...
                ! End of file reached
                    ...
                    PRINT *, 'End of Vehicle Data reached.'
                    GOTO 25
                ENDIF
            ENDIF
        ELSE

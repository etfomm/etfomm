...
! --- Define the width of the upstream intersection.
  ...
  DO CONCURRENT (I = 1: N_STREET_LINKS)
    IREC = RIGHT_LINK(I)
    IF(IREC .EQ. STHRU_LINK(I)) IREC = 0
    IF(IREC .NE. 0) THEN
...
! --- Arrays used for TRAFVU.
 ...
 !LZ 09/03/2025 Potential confilict, hardcoded 7/8 LZ
...
! ----------------------------------------------------------------------
  ...
  IMPLICIT NONE
  CHARACTER*80, INTENT(IN) :: STRING
  INTEGER :: I, IL, IBUF(33), ILINK, ILANE, NLANE, NRTP
! ----------------------------------------------------------------------
...
! --- The number of lanes on RT11 was limited to 9 so add other lane lengths here
    
    DO I = 10, SNUMLANES(IL)
      LANE_LENGTH(IL, I) = LANE_LENGTH(IL, 1)
    ENDDO
    
    NRTP = NUMBER_RIGHTPOCKETS(IL)
    ...
    DO I = NUMBER_RIGHTPOCKETS(IL), 1, -1
      CHANNELIZATION(IL, I) = 4
    ENDDO
    DO I = LAST_FULL_LANE(IL) + 1, TOTAL_LANES(IL)
      CHANNELIZATION(IL, I) = 1
    ENDDO
    ...
    SALIGNMENT_LANE(IL) = SALIGNMENT_LANE(IL) + NUMBER_RIGHTPOCKETS(IL) - NRTP
...
! ----------------------------------------------------------------------
  ...
  ! --- Convert to percentages if necessary.
      ...
      IF(TOTAL .EQ. 0.) THEN
        ...
        CALL SENDTEXTMSG(M_ERROR)
      ELSE
        CALL NORMALIZE5(IBUF(3:7), INPUT)
        LEFT_PERCENT(IL) = INPUT(1)
        STHRU_PERCENT(IL) = INPUT(2)
        RIGHT_PERCENT(IL) = INPUT(3)
      
        IF(DIAG_NODE(IL) .EQ. 0) THEN
          LDIAG_PERCENT(IL) = INPUT(4)
          RDIAG_PERCENT(IL) = INPUT(5)
        ELSEIF(DIAG_NODE(IL) .LT. 0) THEN
          LDIAG_PERCENT(IL) = INPUT(4)
        ELSEIF(DIAG_NODE(IL) .GT. 0) THEN
          RDIAG_PERCENT(IL) = INPUT(4)
        ENDIF
      ENDIF
    ENDIF
  ENDIF
  ...
  ELSE
    ...
    ELSE
      IF(IBUF(3) + IBUF(4) + IBUF(5) + IBUF(6) .GT. 0) THEN
        COND_LEFT(IL) = .TRUE.
        CALL NORMALIZE4(IBUF(3:6), INPUT)
        COND_LEFTPCT(IL, 1) = INPUT(1)
        COND_LEFTPCT(IL, 2) = INPUT(2)
        COND_LEFTPCT(IL, 3) = INPUT(3)
        IF(LDIAG_PERCENT(IL) .NE. 0 .AND. RDIAG_PERCENT(IL) .NE. 0) THEN
          COND_LEFTPCT(IL, 4) = INPUT(4) / 2
          COND_LEFTPCT(IL, 5) = INPUT(4) / 2
        ELSEIF(LDIAG_PERCENT(IL) .NE. 0) THEN
          COND_LEFTPCT(IL, 4) = INPUT(4)
        ELSEIF(RDIAG_PERCENT(IL) .NE. 0) THEN
          COND_LEFTPCT(IL, 5) = INPUT(4)
        ENDIF
      ENDIF
      IF(IBUF(7) + IBUF(8) + IBUF(9) + IBUF(10) .GT. 0) THEN
        COND_THRU(IL) = .TRUE.
        CALL NORMALIZE4(IBUF(7:10), INPUT)
        COND_THRUPCT(IL, 1) = INPUT(1)
        COND_THRUPCT(IL, 2) = INPUT(2)
        COND_THRUPCT(IL, 3) = INPUT(3)
        IF(LDIAG_PERCENT(IL) .NE. 0 .AND. RDIAG_PERCENT(IL) .NE. 0) THEN
          COND_THRUPCT(IL, 4) = INPUT(4) / 2
          COND_THRUPCT(IL, 5) = INPUT(4) / 2
        ELSEIF(LDIAG_PERCENT(IL) .NE. 0) THEN
          COND_THRUPCT(IL, 4) = INPUT(4)
        ELSEIF(RDIAG_PERCENT(IL) .NE. 0) THEN
          COND_THRUPCT(IL, 5) = INPUT(4)
        ENDIF
      ENDIF
      IF(IBUF(11) + IBUF(12) + IBUF(13) + IBUF(14) .GT. 0) THEN
        COND_RIGHT(IL) = .TRUE.
        CALL NORMALIZE4(IBUF(11:14), INPUT)
        COND_RIGHTPCT(IL, 1) = INPUT(1)
        COND_RIGHTPCT(IL, 2) = INPUT(2)
        COND_RIGHTPCT(IL, 3) = INPUT(3)
        IF(LDIAG_PERCENT(IL) .NE. 0 .AND. RDIAG_PERCENT(IL) .NE. 0) THEN
          COND_RIGHTPCT(IL, 4) = INPUT(4) / 2
          COND_RIGHTPCT(IL, 5) = INPUT(4) / 2
        ELSEIF(LDIAG_PERCENT(IL) .NE. 0) THEN
          COND_RIGHTPCT(IL, 4) = INPUT(4)
        ELSEIF(RDIAG_PERCENT(IL) .NE. 0) THEN
          COND_RIGHTPCT(IL, 5) = INPUT(4)
        ENDIF
      ENDIF
      IF(IBUF(15) + IBUF(16) + IBUF(17) + IBUF(18) .GT. 0) THEN
        COND_LDIAG(IL) = .TRUE.
        COND_RDIAG(IL) = .TRUE.
        CALL NORMALIZE4(IBUF(15:18), INPUT)
        ...
        IF(LDIAG_PERCENT(IL) .NE. 0 .AND. RDIAG_PERCENT(IL) .NE. 0) THEN
          COND_LDIAGPCT(IL, 4) = IBUF(4) / 2
          COND_RDIAGPCT(IL, 4) = IBUF(4) / 2
          COND_LDIAGPCT(IL, 5) = IBUF(4) / 2
          COND_RDIAGPCT(IL, 5) = IBUF(4) / 2
        ELSEIF(LDIAG_PERCENT(IL) .NE. 0) THEN
          COND_LDIAGPCT(IL, 4) = IBUF(4)
          COND_RDIAGPCT(IL, 4) = IBUF(4)
        ELSEIF(RDIAG_PERCENT(IL) .NE. 0) THEN
          COND_LDIAGPCT(IL, 5) = IBUF(4)
          COND_RDIAGPCT(IL, 5) = IBUF(4)
        ENDIF
      ENDIF
    ENDIF
  ENDIF
...
! ==================================================================================================
  SUBROUTINE NORMALIZE4(A, B)
  IMPLICIT NONE
  INTEGER, INTENT(IN) :: A(4)
  REAL, INTENT(OUT) :: B(4)
  INTEGER :: I, IMAX, N
  REAL :: TOTAL, ERROR, BMAX
! ----------------------------------------------------------------------
  TOTAL = SUM(A)
  IF(TOTAL .NE. 100.) THEN
    DO I = 1, 5
      B(I) = 100 * FLOAT(A(I)) / TOTAL
    ENDDO
    ERROR = 100.0 - SUM(B)
        ...
        DO IAP = 1, AC_SIGNALS(ISIG)%N_DIRECT_APPROACHES
          ...
          DO PHASE = 1, 8
            IF(AC_SIGNALS(ISIG)%PHASE(PHASE)%IN_USE) THEN
              ...
              IF(AC_SIGNALS(ISIG)%CHECKIN_DISTANCE(PHASE, IAP) .NE. 0) THEN
                AC_SIGNALS(IACT)%CHECKIN_DISTANCE(PHASE, IAP) = AC_SIGNALS(ISIG)%CHECKIN_DISTANCE(PHASE, IAP)
              ENDIF
              IF(AC_SIGNALS(ISIG)%CHECKOUT_DISTANCE(PHASE, IAP) .NE. 0) THEN
                AC_SIGNALS(IACT)%CHECKOUT_DISTANCE(PHASE, IAP) = AC_SIGNALS(ISIG)%CHECKOUT_DISTANCE(PHASE, IAP)
              ENDIF
              
              !IF(AC_SIGNALS(IACT)%CHECKIN_DISTANCE(PHASE, IAP) .GT. 0) THEN
              !  WRITE(MSGTEXT, '(A, I4)') '  APPROACH NUMBER = ', IAP
              !  CALL SENDTEXTMSG(M_INFO)
              !  WRITE(MSGTEXT, '(A, I4)') '  UPNODE = ', SUSN(AC_SIGNALS(IACT)%DIRECT_APPROACH(NAP))
              !  CALL SENDTEXTMSG(M_INFO)
              !  WRITE(MSGTEXT, '(A, I4)') '  SIGNAL NODE = ', SDSN(AC_SIGNALS(IACT)%DIRECT_APPROACH(NAP))
              !  CALL SENDTEXTMSG(M_INFO)
              !  WRITE(MSGTEXT, '(A, I4)') '  PHASE = ', PHASE
              !  CALL SENDTEXTMSG(M_INFO)
              !  WRITE(MSGTEXT, '(A, I5)') '  CHECKIN DISTANCE = ', AC_SIGNALS(IACT)%CHECKIN_DISTANCE(PHASE, IAP)
              !  CALL SENDTEXTMSG(M_INFO)
              !  WRITE(MSGTEXT, '(A, I5)') '  CHECKOUT DISTANCE = ', AC_SIGNALS(IACT)%CHECKOUT_DISTANCE(PHASE, IAP)
              !  CALL SENDTEXTMSG(M_INFO)
              !  WRITE(MSGTEXT, '(A, I4)') '  POLICY = ', AC_SIGNALS(IACT)%POLICY(IAP, PHASE)
              !  CALL SENDTEXTMSG(M_INFO)
              !  MSGTEXT = ''
              !  CALL SENDTEXTMSG(M_INFO)
              !ENDIF
...
! ==================================================================================================
  ...
  REAL :: SPLITS(8)
  INTEGER :: IACT, PHASE
  integer :: il, usn, dsn, iap, dist, ndet, nr, usnx, inode, i, idet, ilink, ilx
! ----------------------------------------------------------------------
  DO IACT = 1, NUMBER_OF_ACS    
    PHASE = AC_SIGNALS(IACT)%PHASE_SEQUENCE(1)
    IF(PHASE .EQ. 0) CYCLE
    IF(AC_SIGNALS(IACT)%PHASE(PHASE)%IN_USE) AC_SIGNALS(IACT)%RING_PHASE_SEQUENCE(1, 1) = PHASE
    
    PHASE = AC_SIGNALS(IACT)%PHASE_SEQUENCE(2)
    IF(PHASE .EQ. 0) CYCLE
    IF(AC_SIGNALS(IACT)%PHASE(PHASE)%IN_USE) AC_SIGNALS(IACT)%RING_PHASE_SEQUENCE(1, 2) = PHASE
    
    PHASE = AC_SIGNALS(IACT)%PHASE_SEQUENCE(3)
    IF(PHASE .EQ. 0) CYCLE
    IF(AC_SIGNALS(IACT)%PHASE(PHASE)%IN_USE) AC_SIGNALS(IACT)%RING_PHASE_SEQUENCE(1, 3) = PHASE
    
    PHASE = AC_SIGNALS(IACT)%PHASE_SEQUENCE(4)
    IF(PHASE .EQ. 0) CYCLE
    IF(AC_SIGNALS(IACT)%PHASE(PHASE)%IN_USE) AC_SIGNALS(IACT)%RING_PHASE_SEQUENCE(1, 4) = PHASE
    
    PHASE = AC_SIGNALS(IACT)%PHASE_SEQUENCE(5)
    IF(PHASE .EQ. 0) CYCLE
    IF(AC_SIGNALS(IACT)%PHASE(PHASE)%IN_USE) AC_SIGNALS(IACT)%RING_PHASE_SEQUENCE(2, 1) = PHASE
    
    PHASE = AC_SIGNALS(IACT)%PHASE_SEQUENCE(6)
    IF(PHASE .EQ. 0) CYCLE
    IF(AC_SIGNALS(IACT)%PHASE(PHASE)%IN_USE) AC_SIGNALS(IACT)%RING_PHASE_SEQUENCE(2, 2) = PHASE
    
    PHASE = AC_SIGNALS(IACT)%PHASE_SEQUENCE(7)
    IF(PHASE .EQ. 0) CYCLE
    IF(AC_SIGNALS(IACT)%PHASE(PHASE)%IN_USE) AC_SIGNALS(IACT)%RING_PHASE_SEQUENCE(2, 3) = PHASE
    
    PHASE = AC_SIGNALS(IACT)%PHASE_SEQUENCE(8)
    IF(PHASE .EQ. 0) CYCLE
    IF(AC_SIGNALS(IACT)%PHASE(PHASE)%IN_USE) AC_SIGNALS(IACT)%RING_PHASE_SEQUENCE(2, 4) = PHASE
  ...
  !Add checkin detectors for trams
  ndet = 0
  do iact = 1, number_of_acs    
    do phase = 1, 8
      do iap = 1, ac_signals(iact)%n_direct_approaches
        if(ac_signals(iact)%checkin_distance(phase, iap) .ne. 0.) then
          ndet = ndet + 1
        endif
      enddo
    enddo
  ...
  if(ndet .gt. 0) then
    !allocate array
    allocate(transit_detector(ndet))
    ndet = 0
    do iact = 1, number_of_acs    
      do phase = 1, 8
        do iap = 1, ac_signals(iact)%n_direct_approaches
          if(ac_signals(iact)%checkin_distance(phase, iap) .ne. 0.) then
            ndet = ndet + 1
            il = ac_signals(iact)%direct_approach(iap)
            dist = ac_signals(iact)%checkin_distance(phase, iap)
            transit_detector(ndet)%signal_node = iact
            transit_detector(ndet)%associated_phase = phase
            if(dist .le. slength(il)) then
              transit_detector(ndet)%location = slength(il) - dist
              ilink = ac_signals(iact)%direct_approach(iap)
            else
              !Find the upstream link
              dist = dist - slength(il)
              usn = susn(il)
              do while(dist .gt. 0)
                do nr = 1, number_of_routes
                  do inode = 1, busr_nnodes(nr) - 1
                    usnx = busr_route_nodes(nr, inode)
                    if(busr_route_nodes(nr, inode + 1) .eq. usn) then
                      call find_street_link(usnx, usn, ilx)
                      if(ilx .ne. 0) then
                        if(slength(ilx) .eq. 0) then
                          transit_detector(ndet)%location = 0
                          ilink = ac_signals(iact)%direct_approach(iap)
                          goto 10
                        elseif(dist .le. slength(ilx)) then
                          transit_detector(ndet)%location = slength(ilx) - dist
                          ilink = ac_signals(iact)%direct_approach(iap)
                          goto 10
                        else
                          il = ilx
                          dist = dist - slength(ilx)
                          usn = usnx
                        endif
                      endif
                    endif    
...
10          continue
            if(ilink .ne. 0) then
              transit_detector(ndet)%link = ilink
              if(sfirst_transit_detector(ilink) .eq. 0) then
                sfirst_transit_detector(ilink) = ndet
              else
                idet = sfirst_transit_detector(ilink)
                do while(idet .ne. 0)
                  i = idet            
                  idet = transit_detector(i)%next_det
                enddo
                transit_detector(i)%next_det = ndet
              endif
            else
              write(msgtext, '(a, i5)') 'PROCESS_ACTUATED_CONTROL::CHECKIN DETECTOR COULD NOT BE LOCATED'
              call sendtextmsg(m_error)
...
! ----------------------------------------------------------------------
  FIRST_FULL_LANE(IL) = FIRST_FULL_LANE(IL) + RTW_LANES(IL)
  LAST_FULL_LANE(IL) = LAST_FULL_LANE(IL) + RTW_LANES(IL)
  TOTAL_LANES(IL) = TOTAL_LANES(IL) + 1
  DO ILN = TOTAL_LANES(IL), RTW_LANES(IL) + 1, -1
    ...
    ENDDO
  ENDDO
  DO ILN = 1, RTW_LANES(IL)
    CHANNELIZATION(IL, ILN) = 4
    LANE_LENGTH(IL, ILN) = SLENGTH(IL) - RTW_EXIT_POINT(IL)
    LANE_NUMBERS(IL, ILN) = 7 - NUMBER_LEFTPOCKETS(IL)
    DO KLN = 1, N_STREET_LANES
      LT_ARC_LENGTH(IL, ILN, KLN) = 0.
      THRU_ARC_LENGTH(IL, ILN, KLN) = 0.
      RT_ARC_LENGTH(IL, ILN, KLN) = 0.
      LD_ARC_LENGTH(IL, ILN, KLN) = 0.
      RD_ARC_LENGTH(IL, ILN, KLN) = 0.
    ENDDO
  ENDDO
  SALIGNMENT_LANE(IL) = SALIGNMENT_LANE(IL) + RTW_LANES(IL)
  DO ILUP = 1, N_STREET_LINKS
    IF(STHRU_LINK(ILUP) .EQ. IL) THEN
      STHRU_ALIGNMENT_LANE(ILUP) = STHRU_ALIGNMENT_LANE(ILUP) + RTW_LANES(IL)
      EXIT
    ENDIF
  ENDDO
  RETURN

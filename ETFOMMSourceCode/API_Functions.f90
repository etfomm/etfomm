...
! ----------------------------------------------------------------------
  ...
  REAL, INTENT(IN) :: TIME
  INTEGER :: ENODE, NETWORK, IL, RDIST, STATUS, IV, CFM
  REAL :: RNDNUM
...
!
! --- Set vehicle ID as the return value to indicate success.
!
  ADD_VEHICLE = IV
  RETURN
  END
...
! ==================================================================================================
  INTEGER FUNCTION GET_USER_DCS_ENTRIES[DLLEXPORT, STDCALL] (I1, I2, I3, D1, D2)
  USE SIMPARAMS
  IMPLICIT NONE
...
! ==================================================================================================
  INTEGER FUNCTION GET_SPLITS_FLAGS[DLLEXPORT, STDCALL] (I1)
  USE SIMPARAMS
  USE ACTUATED_CONTROLLERS
  IMPLICIT NONE
  INTEGER :: I1(NUMBER_OF_ACS)
! ----------------------------------------------------------------------
  I1 = READ_SPLITS
  GET_SPLITS_FLAGS = 0
  END
...
! ==================================================================================================
  INTEGER FUNCTION GET_RUN_INPUTS[DLLEXPORT, STDCALL](R, S1, S2, S3, F1)
  USE SIMPARAMS
  USE SEEDS
  USE FREEWAY_VEHICLES
  USE STREET_VEHICLES
...
! ----------------------------------------------------------------------
  ...
  OUTPUTS%DWELL_MULTIPLIER = DWELL_MULTIPLIER
...
! ----------------------------------------------------------------------
  DO I = 1, NTYPES
    ...
    OUTPUTS(I)%HEADWAY_FACTOR        = HDWY_FACTOR(I)         
    OUTPUTS(I)%AVERAGE_OCCUPANCY     = AVG_OCCS(I)            
    OUTPUTS(I)%EMERGENCY_DECEL       = NEMDEC(I)               
...
! ----------------------------------------------------------------------
  ...
  IF(IL .NE. 0) THEN
    ...
    OUTPUTS%LEFTPCT = COND_LEFTPCT(IL, 1:4)
    OUTPUTS%THRUPCT = COND_THRUPCT(IL, 1:4)
    OUTPUTS%RIGHTPCT = COND_RIGHTPCT(IL, 1:4)
    OUTPUTS%LDIAGPCT = COND_LDIAGPCT(IL, 1:4)
    OUTPUTS%RDIAGPCT = COND_RDIAGPCT(IL, 1:4)
  ENDIF
  END
...
! ----------------------------------------------------------------------
 ...
 !----------------------------------------------------------------------
  DO IDET = 1, N_FREEWAY_DETECTORS
    ...
    OUTPUTS(IDET)%LANE1 = FDETECTOR(IDET)%LANE1
    OUTPUTS(IDET)%LANE2 = FDETECTOR(IDET)%LANE2
...
! ----------------------------------------------------------------------
  DO IL = 1, N_FREEWAY_LINKS
    ...
    LINK_DATA(IL)%HOV_LANES                 = HOV_LANES(IL, 1:3)
...
! ----------------------------------------------------------------------
  ...
  IF(IL .NE. 0) THEN
    ...
    INTERSECTION_DATA%LT_ARC_LENGTH = LT_ARC_LENGTH(IL, 1:N_STREET_LANES, 1:N_STREET_LANES)
    INTERSECTION_DATA%RT_ARC_LENGTH = RT_ARC_LENGTH(IL, 1:N_STREET_LANES, 1:N_STREET_LANES)
...
! ----------------------------------------------------------------------
  DO I = 1, NUMBER_OF_FTCS
    ...
    IF(FTC_SIGNALS(I)%ACTIVE_INTERVALS .EQ. 1) THEN
      IF(FTC_SIGNALS(I)%SIGNAL_CODE(J, 1) .EQ. S_YIELD) THEN
        FTC_SIGNALS(I)%SIGNAL_CODE(J, 1) = 0
      ELSEIF(FTC_SIGNALS(I)%SIGNAL_CODE(J, 1) .EQ. S_PERGRN) THEN
        FTC_SIGNALS(I)%SIGNAL_CODE(J, 1) = 1
      ELSEIF(FTC_SIGNALS(I)%SIGNAL_CODE(J, 1) .EQ. S_STOP) THEN
        FTC_SIGNALS(I)%SIGNAL_CODE(J, 1) = 5
      ENDIF
    ENDIF
  ENDDO
  GET_FTC_SIGNALS = 0
...
! ----------------------------------------------------------------------
  DO IACT = 1, NUMBER_OF_ACS
    ...
    DO PHASE = 1, 8
      ...
      DO IAP = 1, 10
        SIGNALDATA(IACT)%DIAGARROW(PHASE, IAP) = AC_SIGNALS(IACT)%PHASE(PHASE)%LDIAGARROW(IAP) + AC_SIGNALS(IACT)%PHASE(PHASE)%RDIAGARROW(IAP)
      ENDDO
    ENDDO
    ...
    SIGNALDATA(IACT)%WALK_TIME = AC_SIGNALS(IACT)%GUI_NEW_PED_WALK_TIMES
    SIGNALDATA(IACT)%WALK_CLEARANCE_TIME = AC_SIGNALS(IACT)%GUI_NEW_PED_CLEARANCE_TIMES
    SIGNALDATA(IACT)%PED_OMIT = .NOT. AC_SIGNALS(IACT)%GUI_NEW_PED_OMIT
...
! ----------------------------------------------------------------------
  DO I = 1, N_FREEWAY_DETECTORS
    ...
    OUTPUTS(I)%USN = USN
    OUTPUTS(I)%DSN = DSN
    OUTPUTS(I)%LANE1 = FDETECTOR(I)%LANE1 
...
! ----------------------------------------------------------------------
  DO I = 1, N_STREET_DETECTORS
    ...
    OUTPUTS(I)%USN = USN
    OUTPUTS(I)%DSN = DSN
    OUTPUTS(I)%LANE1 = SDETECTOR(I)%LANE1 
    OUTPUTS(I)%LANE2 = SDETECTOR(I)%LANE2 
...
! ----------------------------------------------------------------------
  DO I = 1, NUMBER_OF_PARKING_ZONES
    ...
    OUTPUTS(I)%LINK = PARKING_ZONE_LINK(I) 
  ENDDO
  GET_PARKING_ZONES = 0
  END
...
! ==================================================================================================
  ...
  IMPLICIT NONE
! ----------------------------------------------------------------------
  GET_NUMBER_OF_LONGTERM_EVENTS = NUMBER_OF_EVENTS
  END
...
! ==================================================================================================
  INTEGER FUNCTION GET_EVENTS[DLLEXPORT, STDCALL](OUTPUTS)
  USE API_DATA
  USE EVENTS
  IMPLICIT NONE
...
! ==================================================================================================
  ...
  IF(ALLOCATED(FACCELERATION)) THEN
    DO I = 1, HIGHEST_INDEX_F
      ...
      VDATA(I)%PATHPOINT               = FPATHPOINT(I)
      VDATA(I)%PSAVE                   = FPSAVE(I)
      VDATA(I)%PSEUDO_LEADER           = FPSEUDO_LEADER(I)
      VDATA(I)%PREV_ACCEL              = FPREV_ACCEL(I)
      VDATA(I)%PREVLINK                = FPREVLINK(I)
      VDATA(I)%PREVLANE                = FPREVLANE(I)
      VDATA(I)%ROUTEID                 = FROUTEID(I)
      VDATA(I)%SPEED                   = FSPEED(I)
      VDATA(I)%SPEED_ADJ               = FSPEED_ADJ(I)
      VDATA(I)%TURNCODE                = FTURNCODE(I)
      VDATA(I)%VLENGTH                 = FVLENGTH(I)
      VDATA(I)%VTYPE                   = FVTYPE(I)
      VDATA(I)%XCODE                   = FXCODE(I)
      VDATA(I)%WILL_COOP_EV            = FWILL_COOP_EV(I)
      VDATA(I)%WILL_COOP_LC            = FWILL_COOP_LC(I)
      VDATA(I)%WILL_MOVE               = FWILL_MOVE(I)
      VDATA(I)%DESTINATION             = DESTINATION(I)
      VDATA(I)%DISTANCE_TO_SEGMENT_END = DISTANCE_TO_SEGMENT_END(I)
      VDATA(I)%DIVERTED                = FDIVERTED(I)
      VDATA(I)%HOV_VIOLATOR            = HOV_VIOLATOR(I)
      VDATA(I)%IMETER                  = IMETER(I)
      VDATA(I)%INCIDENT_NUM            = INCIDENT_NUM(I)
      VDATA(I)%ISEGMENT                = ISEGMENT(I)
      VDATA(I)%MUST_MERGE              = MUST_MERGE(I)
      VDATA(I)%NEXT_OBJECT             = NEXT_OBJECT(I)
      VDATA(I)%REMAINING_DIST          = REMAINING_DIST(I)
      VDATA(I)%SORTED_LIST             = SORTED_LIST(I)
      VDATA(I)%SORT_POSITION           = SORT_POSITION(I)
      VDATA(I)%CF_MODEL                = FCF_MODEL(I)
    ENDDO
...
! ----------------------------------------------------------------------
  DO I = 1, NUMBER_OF_INCIDENTS
    OUTPUTS(I)%LINK = INCIDENT_LINK(I) 
...
! ----------------------------------------------------------------------
  DO I = 1, NUMBER_OF_DIVERSIONS
    ...
    OUTPUTS(I)%LINK = DIVERSION_LINK(I)
...
! ----------------------------------------------------------------------
  ...
  IF(TIME_PERIOD .LE. 1) THEN
    DO IL = 1, N_STREET_LINKS
      ...
      !LT_SPEED(IL) = LINK_DATA(IL)%LT_SPEED
      !RT_SPEED(IL) = LINK_DATA(IL)%RT_SPEED
      LT_SPEED(IL) = 22
      RT_SPEED(IL) = 13
      ...
      FIRST_FULL_LANE(IL) = NUMBER_RIGHTPOCKETS(IL) + NUMBER_TURNINGWAYS(IL) + 1
...
! ----------------------------------------------------------------------
  ...
  ELSE
    ...
    IF(TIME_PERIOD .LE. 1 .AND. FIRSTCALL) THEN
      ...
      DO I = 1, NUMBER_OF_ENTRYNODES
        INODE = ENTRY_DATA(I)%SS_USN
        ITYPE = -1
        IF(INODE .NE. 0) ITYPE = NODE_TYPE(INODE)
        IF(ITYPE .EQ. NT_INTERN) THEN
          FOUND = .FALSE.
          DO IL = 1, N_STREET_LINKS
    ...
    ELSE
      ...
      DO I = 1, NUMBER_OF_ENTRYNODES
        ...
        IF(NODE_TYPE(INODE) .NE. NT_EXTERN) THEN
        ...
        DO J = 1, FTC_SIGNALS(I)%APPROACHES
          IF(FTC_SIGNALS(I)%APPROACH(J) .EQ. 0) THEN
            ...
            DO ILX = 1, N_STREET_LINKS
              IF(NODE_TYPE(SUSN(ILX)) .EQ. NT_EXTERN .AND. SDSN(ILX) .EQ. NODE) THEN
                IL = ILX
                EXIT
              ENDIF
            ENDDO
          ...
          ELSE
            ...
            IF(FTC_SIGNALS(I)%ACTIVE_INTERVALS .EQ. 1) THEN
              ...
              ELSEIF(FTC_SIGNALS(I)%SIGNAL_CODE(J, 1) .EQ. 1) THEN
                FTC_SIGNALS(I)%SIGNAL_CODE(J, 1) = S_GREEN
...
! ==================================================================================================
  ...
  !INTEGER FUNCTION SET_NUMBER_OF_AC_SIGNALS[DLLEXPORT, STDCALL](N1, N2) RESULT(ERRORS)
  USE ACTUATED_CONTROLLERS
  USE SIMPARAMS
  USE TEXT
  IMPLICIT NONE
...
! ----------------------------------------------------------------------
  IF(TIME_PERIOD .LE. 1) THEN
    ERRORS = 0
    NUMBER_OF_ACS = N1
    NUMBER_OF_SUPER_ACS = N2
    CALL ALLOCATE_AC_ARRAYS(N1 + N2)
...
! ==================================================================================================
  ...
  INTEGER :: APLINKS(5), LIST(8), NSC
  REAL :: SPLITS(8), FORCEOFFS(8)
  LOGICAL :: IN_USE(8), FOUND
! ----------------------------------------------------------------------
  ERRORS = 0    
  DO IACT = 1, NUMBER_OF_ACS
    IN_USE = .FALSE.
    ...
    ELSE
      ...
      DO IAP = 1, AC_SIGNALS(IACT)%N_DIRECT_APPROACHES
        ...
        DO ILINK = 1, N_STREET_LINKS
          IF(SUSN(ILINK) .EQ. USN .AND. SDSN(ILINK) .EQ. NODE) THEN
            IL = ILINK
            APLINKS(IAP) = IL
            EXIT
          ENDIF
        ...
        IF(IL .EQ. 0) THEN
          ERRORS = ERRORS + 1
          WRITE(MSGTEXT, '(A, I4)') 'APPROACH LINK NOT FOUND FOR ACTUATED SIGNAL AT NODE ', NODE
          CALL SENDTEXTMSG(M_ERROR)
          WRITE(MSGTEXT, '(A, I1, A, I4)') '  APPROACH NUMBER = ', IAP, ' USN = ', USN
          CALL SENDTEXTMSG(M_ERROR)
          EXIT
        ELSE
      ...
      DO IAP = 1, AC_SIGNALS(IACT)%N_INDIRECT_APPROACHES
        ...
        FOUND = .FALSE.
        DO ILINK = 1, N_STREET_LINKS
          IF(SUSN(ILINK) .EQ. USN .AND. SDSN(ILINK) .EQ. DSN) THEN
            AC_SIGNALS(IACT)%INDIRECT_APPROACH(IAP) = ILINK
            FOUND = .TRUE.
            EXIT
          ENDIF
        ENDDO
        IF(.NOT. FOUND) THEN
          ERRORS = ERRORS + 1
          WRITE(MSGTEXT, '(A, I4)') '%INDIRECT APPROACH LINK NOT FOUND FOR ACTUATED SIGNAL AT NODE ', NODE
          CALL SENDTEXTMSG(M_ERROR)
          WRITE(MSGTEXT, '(A, I1, A, I4)') '  APPROACH NUMBER = ', IAP, ' USN = ', USN
          CALL SENDTEXTMSG(M_ERROR)
          EXIT
      ...
      AC_SIGNALS(IACT)%GUI_NEW_PED_WALK_TIMES = AC_DATA_INPUTS(IACT)%WALK_TIME
      AC_SIGNALS(IACT)%GUI_NEW_PED_CLEARANCE_TIMES = AC_DATA_INPUTS(IACT)%WALK_CLEARANCE_TIME
      AC_SIGNALS(IACT)%GUI_NEW_PED_OMIT = .NOT. AC_DATA_INPUTS(IACT)%PED_OMIT
...
! ==================================================================================================
  INTEGER FUNCTION DEFINE_SUPER_CONTROLLER[DLLEXPORT, STDCALL](IBUF) RESULT(ERRORS)
  IMPLICIT NONE
...
! ----------------------------------------------------------------------
  IF(TIME_PERIOD .LE. 1) THEN
    ...
    DO I = 1, NUMBER_OF_ROUTES
      ...
      IF(.NOT. FOUND) THEN
        DO IL = 1, N_STREET_LINKS
          IF(NODE_TYPE(IL) .EQ. NT_EXTERN .AND. SDSN(IL) .EQ. INPUTS(I)%ROUTE_NODES(1)) THEN
            FOUND = .TRUE.
      ...
      IF(NETCODE(LAST) .EQ. I_FREEWAY) THEN
        DO IL = 1, N_FREEWAY_LINKS
          IF(NODE_TYPE(FDSN(IL)) .EQ. NT_EXTERN .AND. FUSN(IL) .EQ. INPUTS(I)%ROUTE_NODES(LAST)) THEN
  ...
  ELSE
    ERRORS = 1
    WRITE(MSGTEXT, '(A)') 'DEFINE_BUSROUTES CANNOT BE CALLED DURING SUBSEQUENT TIME PERIODS'
    CALL SENDTEXTMSG(M_ERROR)
  ENDIF
  END
...
! ----------------------------------------------------------------------
  ...
  ELSE
    ERRORS = 1
    WRITE(MSGTEXT, '(A)') 'DEFINE_BUSSTATIONS CANNOT BE CALLED DURING SUBSEQUENT TIME PERIODS'
    CALL SENDTEXTMSG(M_ERROR)
  ENDIF
  END
...
! ==================================================================================================
  ...
  INTEGER :: I, USN, DSN, IL, IDET, NDET, NCOUNT, ISIG, SIGNAL_NODE
! ----------------------------------------------------------------------
  ERRORS = 0
  IF(TIME_PERIOD .LE. 1) THEN
    DO I = 1, N_STREET_DETECTORS
      ...
      IF(IL .NE. 0) THEN
        ...
        IF(SDETECTOR(I)%ASSOCIATED_PHASE .NE. 0) THEN
          DO ISIG = 1, NUMBER_OF_ACS
            IF(AC_SIGNALS(ISIG)%NODE(1) .EQ. SDETECTOR(I)%SIGNAL_NODE) THEN
...
! ==================================================================================================
  ...
  INTEGER :: I, IDUR, INCTYP, LANES_AFFECTED(N_FREEWAY_LANES)
! ----------------------------------------------------------------------
  ERRORS = 0
  DO I = 1, NUMBER_OF_EVENTS
...
! ----------------------------------------------------------------------
  ...
  EVENT_LINK(NUMBER_OF_EVENTS) = EVENT%LINK
  ...
  ELSE
    ...
    IF(IL .NE. 0) THEN
      ...
      IF(SUM(INPUTS%LDIAGPCT) .NE. 0) THEN
        COND_LDIAG(IL) = .TRUE.
        COND_LDIAGPCT(IL, 1) = INPUTS%LDIAGPCT(1)
        COND_LDIAGPCT(IL, 2) = INPUTS%LDIAGPCT(2)
        COND_LDIAGPCT(IL, 3) = INPUTS%LDIAGPCT(3)
        COND_LDIAGPCT(IL, 4) = INPUTS%LDIAGPCT(4)
      ENDIF
      IF(SUM(INPUTS%RDIAGPCT) .NE. 0) THEN
        COND_RDIAG(IL) = .TRUE.
        COND_RDIAGPCT(IL, 1) = INPUTS%RDIAGPCT(1)
        COND_RDIAGPCT(IL, 2) = INPUTS%RDIAGPCT(2)
        COND_RDIAGPCT(IL, 3) = INPUTS%RDIAGPCT(3)
        COND_RDIAGPCT(IL, 4) = INPUTS%RDIAGPCT(4)
      ENDIF
    ENDIF
  ENDIF
  END
...
! ==================================================================================================
  INTEGER FUNCTION SET_NUMBER_OF_TURNING_WAYS[DLLEXPORT, STDCALL](N) RESULT(ERRORS)
  USE SIMPARAMS
  USE API_DATA
  USE TURNING_WAYS
  USE TEXT
...
! ----------------------------------------------------------------------
  ...
  ELSE
    ERRORS = 1
    WRITE(MSGTEXT, '(A)') 'SET_NUMBER_OF_TURNING_WAYS CANNOT BE CALLED DURING SUBSEQUENT TIME PERIODS'
    CALL SENDTEXTMSG(M_ERROR)
  ENDIF
  END
  
! ==================================================================================================
  INTEGER FUNCTION GET_NUMBER_OF_TURNING_WAYS[DLLEXPORT, STDCALL]()
  USE TURNING_WAYS
  IMPLICIT NONE
! ----------------------------------------------------------------------
  GET_NUMBER_OF_TURNING_WAYS = NUMBER_OF_TURNING_WAYS
  END
  
! ==================================================================================================
  INTEGER FUNCTION DEFINE_TURNING_WAYS[DLLEXPORT, STDCALL](TURNING_WAY_DATA) RESULT(ERRORS)
  USE TURNING_WAYS
  USE STREET_LINKS
  USE NODE_TABLE
  USE GLOBAL_DATA
...
! ----------------------------------------------------------------------
  ...
  ELSE
    ERRORS = 1
    WRITE(MSGTEXT, '(A)') 'DEFINE_TURNING_WAYS CANNOT BE CALLED DURING SUBSEQUENT TIME PERIODS'
    CALL SENDTEXTMSG(M_ERROR)
  ENDIF
  END

! ==================================================================================================
  INTEGER FUNCTION GET_TURNING_WAYS[DLLEXPORT, STDCALL](OUTPUTS) RESULT(ERRORS)
  USE TURNING_WAYS
  USE STREET_LINKS
  USE TEXT
  IMPLICIT NONE
...
10 CONTINUE
  ERRORS = ERRORS + 1
  WRITE(MSGTEXT,'(A)') 'FILE OPEN ERROR: WRITE_TRF_FILE'
  CALL SENDTEXTMSG(M_ERROR)
  WRITE(MSGTEXT, '(A)') ETEXT
  CALL SENDTEXTMSG(M_ERROR)
  RETURN
20 CONTINUE 
  ERRORS = ERRORS + 1
  WRITE(MSGTEXT,'(A)') 'FILE WRITE ERROR: WRITE_TRF_FILE'
  CALL SENDTEXTMSG(M_ERROR)
  WRITE(MSGTEXT, '(A)') ETEXT
  CALL SENDTEXTMSG(M_ERROR)
  CLOSE(100)
...
! ----------------------------------------------------------------------
  ...
  DO IL = 1, N_FREEWAY_LINKS
    ...
    DO I = 1, 3     
      ...
      J = J + 8
    ENDDO
    IF(MAINLINE_RECEIVING_LANE(IL) .GE. 16) THEN
      WRITE(STRING(45:46), '(I2)') MAINLINE_RECEIVING_LANE(IL) - 7
    ELSEIF(MAINLINE_RECEIVING_LANE(IL) .GE. 10) THEN
      WRITE(STRING(45:46), '(I2)') MAINLINE_RECEIVING_LANE(IL) - 5
    ELSE
      WRITE(STRING(45:46), '(I2)') MAINLINE_RECEIVING_LANE(IL)
    ENDIF
    IF(OFFRAMP_SENDING_LANE(IL) .GE. 16) THEN
      WRITE(STRING(47:48), '(I2)') OFFRAMP_SENDING_LANE(IL) - 7
    ELSEIF(OFFRAMP_SENDING_LANE(IL) .GE. 10) THEN
      WRITE(STRING(47:48), '(I2)') OFFRAMP_SENDING_LANE(IL) - 5
    ELSE
      WRITE(STRING(47:48), '(I2)') OFFRAMP_SENDING_LANE(IL)
    ENDIF
    ...
    IF(NODE_TYPE(SDSN(IL)) .NE. NT_EXTERN) THEN
      ...
      IF(WRITE_IT) THEN
        ...
        IF(NODE_TYPE(SUSN(IL)) .NE. NT_EXTERN) THEN
          ...
          WRITE(STRING(21:24), '(I4)') LDIAG_PCT(IL, TIMEPERIOD)
          WRITE(STRING(25:28), '(I4)') RDIAG_PCT(IL, TIMEPERIOD)
        ELSE
...
! ==================================================================================================
  SUBROUTINE WRITE_RT22(TIMEPERIOD)
  USE STREET_LINKS
  IMPLICIT NONE
  INTEGER, INTENT(IN) :: TIMEPERIOD
  CHARACTER*80 STRING
...
! ----------------------------------------------------------------------
  ...
  DO IL = 1, N_STREET_LINKS
    ...
    IF(COND_LEFT(IL) .OR. COND_THRU(IL) .OR. COND_RIGHT(IL) .OR. COND_LDIAG(IL) .OR. COND_RDIAG(IL)) THEN
      WRITE(STRING(1:4), '(I4)') SUSN(IL)
      WRITE(STRING(5:8), '(I4)') SDSN(IL)
      WRITE(STRING(9:12), '(I4)') COND_LEFTPCT(IL, 1)
      WRITE(STRING(13:16), '(I4)') COND_LEFTPCT(IL, 2)
      WRITE(STRING(17:20), '(I4)') COND_LEFTPCT(IL, 3)
      WRITE(STRING(21:24), '(I4)') COND_LEFTPCT(IL, 4)
      WRITE(STRING(25:28), '(I4)') COND_THRUPCT(IL, 1)
      WRITE(STRING(29:32), '(I4)') COND_THRUPCT(IL, 2)
      WRITE(STRING(33:36), '(I4)') COND_THRUPCT(IL, 3)
      WRITE(STRING(37:40), '(I4)') COND_THRUPCT(IL, 4)
      WRITE(STRING(41:44), '(I4)') COND_RIGHTPCT(IL, 1)
      WRITE(STRING(45:48), '(I4)') COND_RIGHTPCT(IL, 2)
      WRITE(STRING(49:52), '(I4)') COND_RIGHTPCT(IL, 3)
      WRITE(STRING(53:56), '(I4)') COND_RIGHTPCT(IL, 4)
      WRITE(STRING(57:60), '(I4)') COND_LDIAGPCT(IL, 1)
      WRITE(STRING(61:64), '(I4)') COND_LDIAGPCT(IL, 2)
      WRITE(STRING(65:68), '(I4)') COND_LDIAGPCT(IL, 3)
      WRITE(STRING(69:72), '(I4)') COND_LDIAGPCT(IL, 4)
      !WRITE(STRING(73:76), '(I4)') COND_RDIAGPCT(IL, 1)
      !WRITE(STRING(77:80), '(I4)') COND_RDIAGPCT(IL, 2)
      !WRITE(STRING(81:84), '(I4)') COND_RDIAGPCT(IL, 3)
      !WRITE(STRING(85:88), '(I4)') COND_RDIAGPCT(IL, 4)
      WRITE(100, '(A)') STRING
    ENDIF
  ENDDO
  RETURN
...
! ----------------------------------------------------------------------
  ...
  DO IDET = 1, N_FREEWAY_DETECTORS
    ...
    WRITE(STRING(13:16), '(I4)') FDETECTOR(IDET)%LOCATION
    ...
    IF(ENTRYNODE_IS_USED(I)) THEN
      ...
      IF(DSN .NE. 0) THEN
        ...
        IF(WRITE_IT) THEN
          ...
          WRITE(STRING(9:12), '(I4)') FLOW_RATE(I, TIMEPERIOD)
...
! ==================================================================================================
  SUBROUTINE WRITE_RT51(TIMEPERIOD)
  USE STREET_LINKS
  USE FLOWDATA_MOD
  USE GLOBAL_DATA
  USE SIMPARAMS
  ...
  INTEGER, INTENT(IN) :: TIMEPERIOD
  CHARACTER*80 STRING
  INTEGER :: IN, IL, I, T1, T2, K, IND, V(8), T(8)
! ----------------------------------------------------------------------
  STRING(79:80) = '51'
  ...
  DO IN = 1, NUMBER_OF_ENTRYNODES
    IF(ENTRYLINK(IN)%NETWORK .EQ. I_STREET) THEN
      ...
      IF(NODE_TYPE(SUSN(IL)) .NE. NT_EXTERN) THEN
        STRING(1:78) = ''
        K = 0
        DO IND = 1, ENTRYLINK(IN)%INDEX
          IF(ENTRYLINK(IN)%TIME(IND) .GE. T1 .AND. ENTRYLINK(IN)%TIME(IND) .LT. T2) THEN
            K = K + 1
            IF(K .GT. 8) EXIT
            V(K) = ENTRYLINK(IN)%FLOW(IND)
            T(K) = ENTRYLINK(IN)%TIME(IND)
          ENDIF
        ENDDO
        IF(K .GT. 0) THEN
          WRITE(STRING(1:76), '(19I4)') CENTROID_LABEL(IL), SUSN(IL), SDSN(IL), (V(I), T(I), I = 1, K) 
          WRITE(100, '(A)') STRING
        ENDIF
      ENDIF
    ENDIF
...
! ----------------------------------------------------------------------
  ...
  DO ITYPE = 1, 9
    ...
    WRITE(STRING(13:16), '(I4)') NEMDEC(ITYPE)
  ...
  IF(ALLOCATED(BUS_STATION_LIST)) THEN
    DO NS = 1, 99
      IF(BUS_STATION_LIST(NS)%LINK .NE. 0) THEN
        ...
        WRITE(STRING(6:8), '(I3)') BUS_STATION_LIST(NS)%DWELL
        WRITE(STRING(9:12), '(I4)') BUS_STATION_LIST(NS)%BYPASS_PCT
        WRITE(100, '(A)') STRING
      ENDIF
    ENDDO
  ENDIF
...
! ----------------------------------------------------------------------
  ...
  DO NR = 1, NUMBER_OF_ROUTES
    IF(BUSR_NNODES(NR) .GT. 0) THEN
      STRING(1:77) = ''
      WRITE(STRING(2:4), '(I3)') NR
      WRITE(STRING(5:8), '(I4)') BUSR_HDWY(NR)
      WRITE(STRING(9:12), '(I4)') BUSR_OFFSET(NR)
      WRITE(100, '(A)') STRING
    ENDIF
  ENDDO
  RETURN
